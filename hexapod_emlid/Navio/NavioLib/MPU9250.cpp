...
bool MPU9250::initialize(int sample_rate_div, uint8_t low_pass_filter) {
	uint8_t i = 0;
...
void MPU9250::getMotion9(ch::sharpsoft::hexapod::transfer::IMU *imu) {
	read_all();
	imu->set_ax(accelerometer_data[0]);
	imu->set_ay(accelerometer_data[1]);
	imu->set_az(accelerometer_data[2]);
	imu->set_gx(gyroscope_data[0]);
	imu->set_gy(gyroscope_data[1]);
	imu->set_gz(gyroscope_data[2]);
	imu->set_mx(magnetometer_data[0]);
	imu->set_my(magnetometer_data[1]);
	imu->set_mz(magnetometer_data[2]);
}

//-----------------------------------------------------------------------------------------------

void MPU9250::getMotion6(float *ax, float *ay, float *az, float *gx, float *gy,
		float *gz) {
	read_acc();
	read_gyro();
	*ax = accelerometer_data[0];
